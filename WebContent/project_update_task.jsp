...
<body class="container-fluid">
	<%	
		...
		String startDate = tVO.getStartDate();
		String deadLine = tVO.getDeadline();
	...
	<div class="row">
		...
		<div class="col-sm-10 ml-auto py-4">
			<!-- 화면 -->
			<!-- 프로젝트 생성창 -->
			<h3 class="text-center">업무 수정</h3>
			<hr>
			<div class="center-block">
				<form action="task_update.jsp?tid=<%= taskId %>" method="post" class="form-horizontal">
					...
					<div class="form-group row">
						...
						<div class="col-md-10">
							<textarea class="form-control" rows="17"
								name = "taskContent"><%= content %></textarea>
						</div>
					</div>
					<div class="form-group row">
						...
						<div class="col-md-10">
							<select name="worker" class="form-control title"">
								<option value="option0" disabled>===담당자 설정===</option>
								<%
									String select = null;
									for(int i = 0; i < members.size(); i++){
										if(worker.equals(members.get(i).getUserId())){
											 select = "select";
										} else select = "else";
								%>
								<option value="<%= members.get(i).getUserId() %> <%=select %>" <%= select %>><%= members.get(i).getUserId() %></option>
								<%} %>
							</select>
						</div>
					</div>
						...
						<div class="col-md-10 row">
							<div class="col-md-3">
								<input type="text" id="from" name="from" class="form-control"
									placeholder="MM/DD/YYYY" value="<%= startDate %>">
							</div>
							<div class="col-md-2">
								<label> ~ </label>
							</div>
							<div class="col-md-3">
								<input type="text" id="to" name="to" class="form-control"
									placeholder="MM/DD/YYYY" value ="<%= deadLine %>">
							</div>
						</div>
					</div>
